...
(run "if zero? (1) then 3 else 10")

; (display (expval->string (value-of (str-exp "sdsa") '())))
...
(str-test3 "if zero?(0) then 'zero' else 'nonzero'" "'zero'")
;

(rev-test "reverse 'co'" "'oc'")
(rev-test2 "reverse 'comp301'" "'103pmoc'")
(rev-test3 "reverse if zero?(op(3, 2, 1)) then 'zero' else 'nonzero' " "'oreznon'")
 )